...
def main():
    ...
    for joint_name, joint_id in hwi.joints.items():
        ...
        try:
            ...
            position = hwi.get_motor_position(joint_id)
            print(f"✓ Motor '{joint_name}' (ID: {joint_id}) is responsive. Position: {position:.3f}")
        except Exception as e:
            ...
            try:
                ...
                for joint_name, joint_id in hwi.joints.items():
                    if (joint_name, joint_id) not in unresponsive_motors:
                        try:
                            hwi.set_torque_enabled(joint_id, False)
    ...
    for joint_name, joint_id in hwi.joints.items():
        ...
        try:
            ...
            current_position = hwi.get_motor_position(joint_id)
